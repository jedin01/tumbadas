...
import { fetchSingleFuncionario } from "../api/ApiCollection";
import {
  LineChart,
  Line,
  XAxis,
...
const Funcionario = () => {
  ...
  if (!id) {
    return (
      <div className="text-center mt-10 text-red-500 font-semibold">
        Nenhum funcionarios especificado.
      </div>
    );
  }
  ...
  const { isLoading, isError, data, isSuccess } = useQuery({
    ...
    queryFn: () => fetchSingleFuncionario(id),
    enabled: !!id,
  });

  React.useEffect(() => {
    if (isLoading) toast.loading("Carregando...", { id: "funcionariosRead" });
    if (isError)
      toast.error("Erro ao carregar dados!", { id: "funcionariosRead" });
    if (isSuccess)
  ...
  return (
    <div id="singleFuncionarios" className="w-full p-0 m-0">
      {isError && (
        <div className="text-center mt-10 text-red-500 font-semibold">
          Erro ao carregar o funcionarios.
        </div>
      )}
            ...
            <div className="w-full flex flex-col items-start gap-5">
              ...
              <div className="flex items-center gap-8 mb-4">
                <div className="flex flex-col items-start gap-1">
                  ...
                  <span className="font-normal text-base">Funcionarios</span>
                </div>
              </div>
