...
import { fetchSingleCliente } from "../api/ApiCollection";
import {
  LineChart,
  Line,
  XAxis,
...
const Cliente = () => {
  ...
  const { isLoading, isError, data, isSuccess } = useQuery({
    ...
    queryFn: () => fetchSingleCliente(id),
    enabled: !!id,
  });

  React.useEffect(() => {
    if (isLoading) toast.loading("Carregando...", { id: "clientesRead" });
    if (isError) toast.error("Erro ao carregar dados!", { id: "clientesRead" });
    if (isSuccess)
      toast.success("Dados carregados com sucesso!", { id: "clientesRead" });
  ...
  return (
    <div id="singleClientes" className="w-full p-0 m-0">
      {isError && (
        <div className="text-center mt-10 text-red-500 font-semibold">
          Erro ao carregar o clientes.
        </div>
      )}
            ...
            <div className="w-full flex flex-col items-start gap-5">
              ...
              <div className="flex items-center gap-8 mb-4">
                <div className="flex flex-col items-start gap-1">
                  ...
                  <span className="font-normal text-base">Clientes</span>
                </div>
              </div>
                ...
                {isSuccess ? (
                  <div className="w-full grid grid-cols-3 xl:flex gap-5 xl:gap-8">
                    ...
                    <div className="col-span-2 flex flex-col font-semibold gap-3">
                      <span>{data.id ?? "-"}</span>
                      <span>{data.pNome ?? "-"}</span>
                      <span>{data.uNome ?? "-"}</span>
