...
const EditFornecedores = () => {
  ...
  const [data, setData] = useState<Fornecedores>({
    id: "",
    pNome: "",
    uNome: "",
    municipio: "",
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/fornecedores/${id}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast.success("Fornecedores atualizado com sucesso!");
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4">Editar Fornecedores</h2>
