...
import { fetchSingleCategoria } from "../api/ApiCollection";
import {
  LineChart,
  Line,
  XAxis,
...
const Categoria = () => {
  ...
  if (!id) {
    return (
      <div className="text-center mt-10 text-red-500 font-semibold">
        Nenhum categorias especificado.
      </div>
    );
  }
  ...
  const { isLoading, isError, data, isSuccess } = useQuery({
    ...
    queryFn: () => fetchSingleCategoria(id),
    enabled: !!id,
  });

  React.useEffect(() => {
    if (isLoading) toast.loading("Carregando...", { id: "categoriasRead" });
    if (isError)
      toast.error("Erro ao carregar dados!", { id: "categoriasRead" });
    if (isSuccess)
      toast.success("Dados carregados com sucesso!", { id: "categoriasRead" });
  ...
  return (
    <div id="singleCategorias" className="w-full p-0 m-0">
      {isError && (
        <div className="text-center mt-10 text-red-500 font-semibold">
          Erro ao carregar o categorias.
        </div>
      )}
            ...
            <div className="w-full flex flex-col items-start gap-5">
              ...
              <div className="flex items-center gap-8 mb-4">
                <div className="flex flex-col items-start gap-1">
                  ...
                  <span className="font-normal text-base">Categorias</span>
                </div>
              </div>
