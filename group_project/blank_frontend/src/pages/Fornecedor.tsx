...
import { fetchSingleFornecedor } from "../api/ApiCollection";
import {
  LineChart,
  Line,
  XAxis,
...
const Fornecedor = () => {
  ...
  if (!id) {
    return (
      <div className="text-center mt-10 text-red-500 font-semibold">
        Nenhum fornecedores especificado.
      </div>
    );
  }
  ...
  const { isLoading, isError, data, isSuccess } = useQuery({
    ...
    queryFn: () => fetchSingleFornecedor(id),
    enabled: !!id,
  });

  React.useEffect(() => {
    if (isLoading) toast.loading("Carregando...", { id: "fornecedoresRead" });
    if (isError)
      toast.error("Erro ao carregar dados!", { id: "fornecedoresRead" });
    if (isSuccess)
  ...
  return (
    <div id="singleFornecedores" className="w-full p-0 m-0">
      {isError && (
        <div className="text-center mt-10 text-red-500 font-semibold">
          Erro ao carregar o fornecedores.
        </div>
      )}
            ...
            <div className="w-full flex flex-col items-start gap-5">
              ...
              <div className="flex items-center gap-8 mb-4">
                <div className="flex flex-col items-start gap-1">
                  ...
                  <span className="font-normal text-base">Fornecedores</span>
                </div>
              </div>
