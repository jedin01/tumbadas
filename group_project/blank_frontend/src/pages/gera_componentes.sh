...
template() {
  local entidade=$1
  local entidadeCap="${entidade^}"    # primeira letra maiúscula
  local camposEnt="${campos[$entidade]}"
...
import { fetchSingle${entidadeCap} } from "../api/ApiCollection";
import {
  LineChart,
  Line,
  XAxis,
...
const ${entidadeCap} = () => {
  ...
  const { isLoading, isError, data, isSuccess } = useQuery({
    ...
    queryFn: () => fetchSingle${entidadeCap}(id),
    enabled: !!id,
  });

  React.useEffect(() => {
    if (isLoading) toast.loading("Carregando...", { id: "${entidade}Read" });
    if (isError) toast.error("Erro ao carregar dados!", { id: "${entidade}Read" });
    if (isSuccess)
      toast.success("Dados carregados com sucesso!", { id: "${entidade}Read" });
  ...
  return (
    <div id="single${entidadeCap}" className="w-full p-0 m-0">
      {isError && (
        <div className="text-center mt-10 text-red-500 font-semibold">
          Erro ao carregar o ${entidade}.
        </div>
      )}
            ...
            <div className="w-full flex flex-col items-start gap-5">
              ...
              <div className="flex items-center gap-8 mb-4">
                <div className="flex flex-col items-start gap-1">
                  ...
                  <span className="font-normal text-base">${entidadeCap}</span>
                </div>
              </div>
...
$(for campo in $camposEnt; do echo "                      <span>${campo^}</span>"; done)
                    </div>
                    <div className="col-span-2 flex flex-col font-semibold gap-3">
$(for campo in $camposEnt; do echo "                      <span>{data.${campo} ?? \"-\"}</span>"; done)
                    </div>
                  </div>
                ) : (
...
export default ${entidadeCap};
EOF
}

for entidade in "${!campos[@]}"; do
  arquivo="${entidade^}.tsx"
  echo "Criando arquivo $arquivo ..."
  template "$entidade" > "$arquivo"
done
